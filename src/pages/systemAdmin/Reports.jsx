...
const Reports = () => {
  ...
  const generateRandomReportData = () => {
    ...
    const generateRandomChartData = () => {
      ...
      return {
        ...
        pieChartData: {
          labels: ["Maintenance", "Repair", "CarWash"],
          datasets: [
            {
              data: randomPieData.map((value) => Math.round((value / totalPie) * 100)),
              backgroundColor: ["#FF6384", "#36A2EB", "#FFCE56"],
    ...
    const serviceTypes = ["Maintenance", "CarWash", "All Services"]; // List of service types
    const selectedServiceType = serviceTypes[randomNumber(0, serviceTypes.length - 1)]; // Randomly select one
  
    return {
      id: Date.now() + randomNumber(1, 1000), // Ensure unique IDs
      title: `Report ${randomNumber(1, 100)}`,
      startDate: `2024-${randomNumber(1, 6).toString().padStart(2, "0")}-01`,
      endDate: `2024-${randomNumber(7, 12).toString().padStart(2, "0")}-01`,
      serviceType: selectedServiceType, // Use the randomly selected service type
      ...generateRandomChartData(),
  ...
  // Initialize reports with six random reports if not present in localStorage
  useEffect(() => {
    const savedReports = localStorage.getItem("reports");
    if (savedReports) {
      setReports(JSON.parse(savedReports));
    } else {
      const initialReports = Array.from({ length: 6 }, generateRandomReportData);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reports.map((report) => (
            ...
            >
              ...
              <p>
                <strong>Service Type:</strong><p>All services</p>
              </p>
